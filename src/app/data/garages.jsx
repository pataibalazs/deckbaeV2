export const products = {
  modena: {
    name: "Modena",
    basePrice: 13959000,
    price: "Bruttó: 13.959.000-től",
    rating: 5,
    images: [
      {
        id: 1,
        name: "Modena",
        src: "/pictures/garages/modena/haz1.webp",
        alt: "Modena prémium garázs – modern könnyűszerkezetes autóbeálló tégla vagy kő burkolattal",
      },
      {
        id: 2,
        name: "Modena",
        src: "/pictures/garages/modena/haz2.webp",
        alt: "Modena prémium garázs – modern könnyűszerkezetes autóbeálló tégla vagy kő burkolattal",
      },

      {
        id: 4,
        name: "Modena",
        src: "/pictures/garages/modena/1.webp",
        alt: "Modena prémium garázs tervrajz és alaprajz",
      },
      {
        id: 5,
        name: "Modena",
        src: "/pictures/garages/modena/11.webp",
        alt: "Modena prémium garázs valós fotó természetes környezetben",
      },
      {
        id: 6,
        name: "Modena",
        src: "/pictures/garages/modena/2.webp",
        alt: "Modena garázs valós fotó modern homlokzattal",
      },
      {
        id: 7,
        name: "Modena",
        src: "/pictures/garages/modena/3.webp",
        alt: "Modena prémium garázs valós fotó kültéri részletekkel",
      },
      {
        id: 8,
        name: "Modena",
        src: "/pictures/garages/modena/4.webp",
        alt: "Modena garázs valós fotó komplett megjelenéssel",
      },
      {
        id: 9,
        name: "Modena",
        src: "/pictures/garages/modena/5.webp",
        alt: "Modena prémium garázs valós fotó környezeti illeszkedéssel",
      },
      {
        id: 10,
        name: "Modena",
        src: "/pictures/garages/modena/6.webp",
        alt: "Modena garázs valós fotó teljes építészeti megoldással",
      },
      {
        id: 3,
        name: "Modena",
        src: "/pictures/garages/modena/haz3.webp",
        alt: "Modena prémium garázs – modern könnyűszerkezetes autóbeálló tégla vagy kő burkolattal",
      },
    ],
    colors: [
      {
        name: "Washed Black",
        bgColor: "bg-gray-700",
        selectedColor: "ring-gray-700",
      },
      { name: "White", bgColor: "bg-white", selectedColor: "ring-gray-400" },
    ],
    description: `
        <p>A Modena prémium garázsmegoldásként ötvözi a funkcionalitást és az esztétikát. Autóbeállóként és garázsként is használható, külső falburkolata vakolt. A teljes elektromos rendszerrel és világítással felszerelt garázs tökéletes választás a modern igényekhez.</p>
      `,
    details: [
      {
        name: "Jellemzők",
        items: [
          "Autóbeálló: 4m x 7m",
          "Garázs: 7m x 4,6m",
          "Beton alap sáv alapozással",
          "Komplett világítás és elektromos rendszer",
        ],
      },
    ],
    extras: [],
  },

  munich: {
    name: "Munich",
    basePrice: 9759000,
    price: "Bruttó: 9.759.000-től",
    rating: 4,
    images: [
      {
        id: 1,
        name: "Munich",
        src: "/pictures/garages/munich/haz1.webp",
        alt: "Munich garázs – tárolós modern könnyűszerkezetes autóbeálló",
      },
      {
        id: 2,
        name: "Munich",
        src: "/pictures/garages/munich/haz2.webp",
        alt: "Munich garázs – tárolós modern könnyűszerkezetes autóbeálló",
      },

      {
        id: 4,
        name: "Munich",
        src: "/pictures/garages/munich/1.webp",
        alt: "Munich garázs valós fotó természetes környezetben",
      },
      {
        id: 5,
        name: "Munich",
        src: "/pictures/garages/munich/2.webp",
        alt: "Munich garázs valós fotó modern autóbeállóval",
      },
      {
        id: 6,
        name: "Munich",
        src: "/pictures/garages/munich/3.webp",
        alt: "Munich garázs valós fotó tárolóval kombinálva",
      },
      {
        id: 7,
        name: "Munich",
        src: "/pictures/garages/munich/4.webp",
        alt: "Munich garázs valós fotó komplett megjelenéssel",
      },
      {
        id: 3,
        name: "Munich",
        src: "/pictures/garages/munich/haz3.webp",
        alt: "Munich garázs – tárolós modern könnyűszerkezetes autóbeálló",
      },
    ],
    colors: [
      {
        name: "Washed Black",
        bgColor: "bg-gray-700",
        selectedColor: "ring-gray-700",
      },
      { name: "White", bgColor: "bg-white", selectedColor: "ring-gray-400" },
    ],
    description: `
        <p>A Munich autóbeálló modern megjelenésével és praktikus kialakításával tökéletes választás mindennapi használatra. Tárolóval kombinálva extra helyet kínál, és teljesen felszerelhető elektromos rendszerrel. A burkolatok és alapozás kialakítása magas szintű tartósságot biztosít.</p>
      `,
    details: [
      {
        name: "Jellemzők",
        items: [
          "Autóbeálló: 7m x 4m",
          "Tároló: 7m x 2,1m",
          "Beton alap, sáv alapozással",
          "Komplett világítás és elektromos rendszer",
        ],
      },
    ],
    extras: [],
  },

  tihany: {
    name: "Tihany",
    basePrice: 11259000,
    price: "Bruttó: 11.259.000-től",
    rating: 4,
    images: [
      {
        id: 1,
        name: "Tihany",
        src: "/pictures/garages/tihany/haz1.webp",
        alt: "Tihany garázs – nemes vakolatú, szigetelhető könnyűszerkezetes autóbeálló",
      },

      {
        id: 3,
        name: "Tihany",
        src: "/pictures/garages/tihany/haz3.webp",
        alt: "Tihany garázs – nemes vakolatú, szigetelhető könnyűszerkezetes autóbeálló",
      },
      {
        id: 4,
        name: "Tihany",
        src: "/pictures/garages/tihany/1.webp",
        alt: "Tihany garázs valós fotó természetes környezetben",
      },
      {
        id: 5,
        name: "Tihany",
        src: "/pictures/garages/tihany/2.webp",
        alt: "Tihany garázs valós fotó nemes vakolatú külső fallal",
      },
      {
        id: 2,
        name: "Tihany",
        src: "/pictures/garages/tihany/haz2.webp",
        alt: "Tihany garázs – nemes vakolatú, szigetelhető könnyűszerkezetes autóbeálló",
      },
    ],
    colors: [
      {
        name: "Washed Black",
        bgColor: "bg-gray-700",
        selectedColor: "ring-gray-700",
      },
      { name: "White", bgColor: "bg-white", selectedColor: "ring-gray-400" },
    ],
    description: `
        <p>A Tihany autóbeálló nemes vakolatú külső falával és letisztult dizájnjával elegáns megjelenést biztosít. Beton alapra épül, kérhető járólap burkolattal, és akár szigetelt helységként is kialakítható.</p>
      `,
    details: [
      {
        name: "Jellemzők",
        items: [
          "Nemes vakolatú külső fal",
          "Beton alap, sáv alapozással",
          "Opcionális járólap burkolat",
          "Akár szigetelt helységként is kérhető",
        ],
      },
    ],
    extras: [],
  },

  toronto: {
    name: "Toronto",
    basePrice: 6755000,
    price: "Bruttó: 6.755.000-től",
    rating: 5,
    images: [
      {
        id: 1,
        name: "Toronto",
        src: "/pictures/garages/toronto/haz1.webp",
        alt: "Toronto garázs – masszív, LED világítással és napelemes tetővel ellátott könnyűszerkezetes autóbeálló",
      },

      {
        id: 3,
        name: "Toronto",
        src: "/pictures/garages/toronto/haz3.webp",
        alt: "Toronto garázs – masszív, LED világítással és napelemes tetővel ellátott könnyűszerkezetes autóbeálló",
      },
      {
        id: 2,
        name: "Toronto",
        src: "/pictures/garages/toronto/haz2.webp",
        alt: "Toronto garázs – masszív, LED világítással és napelemes tetővel ellátott könnyűszerkezetes autóbeálló",
      },
    ],
    colors: [
      {
        name: "Washed Black",
        bgColor: "bg-gray-700",
        selectedColor: "ring-gray-700",
      },
      { name: "White", bgColor: "bg-white", selectedColor: "ring-gray-400" },
    ],
    description: `
        <p>A Toronto autóbeálló modern, masszív kivitelű, LED-világítással és akár napelemes tetőszerkezettel is rendelhető. Strapabíró oszlopszerkezete biztosítja a hosszú élettartamot, míg az alapozás és méretezés révén ideális választás nagyobb járművek számára is.</p>
      `,
    details: [
      {
        name: "Jellemzők",
        items: [
          "Méret: 6,1m x 6,7m",
          "Autóbeálló beton alapozással",
          "15x15-oszlopok, 30x15 gerendák",
          "Opcionális LED-világítás",
          "Napelemmel is telepíthető tetőszerkezet",
        ],
      },
    ],
    extras: [],
  },
};
